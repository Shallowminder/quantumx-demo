  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {suggestedTopics.length > 0 && (
            <span className="flex flex-wrap items-center gap-1">
              ...
              {suggestedTopics.map((topic) => (
                <span
                  key={topic.id}
                  className="theme-surface-ghost rounded-full px-2.5 py-1 text-[11px] text-muted"
                >
                  {topic.name}
                </span>
              ))}
        ...
        <button
          className="theme-primary-button inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition"
          disabled={!canSubmit}
          type="button"
          onClick={submit}
        >
